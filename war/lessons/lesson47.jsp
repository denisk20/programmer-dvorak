<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core"%>
<%@ taglib prefix="typingtutor" uri="/WEB-INF/tlds/typingtutor"%>
<!DOCTYPE html PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN" "http://www.w3.org/TR/html4/loose.dtd">
<html>
	<head>
		<typingtutor:head />
		<title>Programmer Dvorak typing tutorial: Lesson 47: Introducing 7 and 6: Number row, fourth fingers</title>
	</head>
	<body>
		<div class="content">
			<h1>Lesson 47: Introducing 7 and 6: Number row, fourth fingers</h1>
			<div class="nav">
				<a href="/lessons/lesson46.jsp"><img src="/img/left-arrow.png"
													 alt="Previous lesson"
													 title="Lesson 46: Comprehensive, including 5 and 4"/></a>
				<a href="/index.html" class="toc"><img src="/img/toc.png"
													   alt="Table of contents" title="Table of contents" /></a>
				<a href="/lessons/lesson48.jsp"><img src="/img/right-arrow.png"
												  alt="Next lesson" title="Lesson 48: Comprehensive, including 7 and 6"/></a>
			</div>
			<typingtutor:typingtutor>
				7777 6666 7777 6666 7777 6666 7777 6666
				7777 6666 7777 6666 7777 6666 7777 6666
				7 7 7 7 6 6 6 6 76 76 76 76 667 667 667 667
				6 76 6 76 6 76 6 76 676 676 676 676
				7 676 76 7 676 76 7 676 76 7 676 76 
			</typingtutor:typingtutor>
		</div>
	</body>
</html>