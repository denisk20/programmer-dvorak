<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core"%>
<%@ taglib prefix="typingtutor" uri="/WEB-INF/tlds/typingtutor"%>
<!DOCTYPE html PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN" "http://www.w3.org/TR/html4/loose.dtd">
<html>
	<head>
		<typingtutor:head />
		<title>Programmer Dvorak typing tutorial: Lesson 38: Comprehensive, including & and !</title>
	</head>
		<body> 		<typingtutor:keyboard/> 
		<div class="content">
			<h1>Lesson 38: Comprehensive, including & and !</h1>
			<div class="nav">
				<a href="/lessons/lesson37.jsp"><img src="/img/left-arrow.png"
													 alt="Previous lesson"
													 title="Lesson 37: introducing & and !: Number row, fourth fingers"/></a>
				<a href="/index.html#lessons" class="toc"><img src="/img/toc.png"
													   alt="Table of contents" title="Table of contents" /></a>
				<a href="/lessons/lesson39.jsp"><img src="/img/right-arrow.png"
												  alt="Next lesson" title="Lesson 39: introducing $ and #: Number row, fourth fingers"/></a>
			</div>
			<typingtutor:typingtutor>
				(((( ==== **** }}}} )))) {{{{ ++++ [[[[ ]]]] &&&& !!!!
				(((( ==== **** }}}} )))) {{{{ ++++ [[[[ ]]]] &&&& !!!!
			</typingtutor:typingtutor>
			<typingtutor:typingtutor>
				!a=b++ !a=b++ !a=b++ !a=b++
				*x={call(&a &b)}()++ *x={call(&a &b)}()++ *x={call(&a &b)}()++ *x={call(&a &b)}()++
				&z*=[!a !b !c] &z*=[!a !b !c] &z*=[!a !b !c] &z*=[!a !b !c]
				func(&x, &y){&x=!&y} func(&x, &y){&x=!&y} func(&x, &y){&x=!&y} func(&x, &y){&x=!&y}
				{a,b}->{[a*b]} != &z {a,b}->{[a*b]} != &z {a,b}->{[a*b]} != &z {a,b}->{[a*b]} != &z
				!((var++)*(x[z+y]))->{t} !((var++)*(x[z+y]))->{t} !((var++)*(x[z+y]))->{t} !((var++)*(x[z+y]))->{t}
			</typingtutor:typingtutor>
		</div>
	</body>
</html>